...
import { ChevronDown, ChevronRight, Bot } from "lucide-solid"
...
const SubAgentGroup: Component<SubAgentGroupProps> = (props) => {
  ...
  const [groupExpanded, setGroupExpanded] = createSignal(true) // Default expanded
  ...
  const hasRunning = createMemo(() => aggregateStatus().running > 0)
  ...
  // For multiple agents, render group
  return (
    <div class="subagent-group">
      <button
        ...
        onClick={toggleGroup}
        aria-expanded={groupExpanded() || hasRunning()}
      >
        <span class="subagent-group-chevron">
          {groupExpanded() || hasRunning() ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
        </span>
        <Bot class="subagent-group-icon" size={16} />
        <span class="subagent-group-title">{props.tools.length} Sub-Agents</span>
        <span class="subagent-group-status">{statusSummary()}</span>
      </button>

      <Show when={groupExpanded() || hasRunning()}>
